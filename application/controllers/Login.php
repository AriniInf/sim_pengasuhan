...
class Login extends CI_Controller {
	...
	public function login_validation(){
		...
		if($this->form_validation->run())
		{
			...
			$validateTarunaLogin = $this->model_login->taruna_login($username,$password);
			...
			else if($validateTarunaLogin['status'] === true)
			{
				$session_data = array(
					'username' => $validateTarunaLogin['data'],
					'status' => $validateTarunaLogin['status']
					);
				$this->session->set_userdata($session_data);
				redirect(base_url() . 'login/dashboard_taruna');
			}
	...
	public function dashboard_taruna(){
		$username = $this->session->userdata('username');
		 if($username==""){
		  redirect('login/index');
		 }
		 else{
		  redirect('main/dashboard/dashtaruna');
		}
   }
